...
|---|---|---|---|---|
| Session_01 | `Session_01.psyexp` | 情绪面孔-词冲突任务（训练+Part A+Part B）+ 闭眼/睁眼静息态 | **主任务**，评估情绪冲突加工和认知控制 | ~20 min |
...
| 10 | 正式试次 Part B | MainISI→MainStim→MainResp→MainITI | 96 个冲突判断 trial（Part B） | ~8 min |
...
### 2.4 正式试次时间结构（MainISI → MainStim → MainResp → MainITI）

| Routine | 组件 | 内容 | 持续时长 |
|---|---|---|---|
| MainISI | text_21 ("+") | 注视点 | 0.5 s |
| | text_22 (blank) | jitter 间隔 | $jitter（变量） |
| MainStim | image_2 (面孔图片) | 面孔，尺寸 (0.548, 0.8) | 1.0 s |
| | text_16 (情绪词) | 词语叠加于面孔上 | 1.0 s |
| | serialPort_2 | SerialOut 标记 (3→4, 1.0s) | 1.0 s |
| MainResp | text_17 | "正性面孔→f键 / 负性面孔→j键" | 1.0 s |
| | key_resp_10 | 按键 'f' / 'j'，forceEndRoutine | ≤1.0 s |
| | serialPort_3 | SerialOut 标记 (5→6, 按键触发) | condition |
...
## 附录 A：Session_01 事件标记（SerialOut）

| Routine | startDataNumeric | stopDataNumeric | stopType | 说明 |
|---|---|---|---|---|
| Resting_EC | 1 (r) | 120 (x) | duration=180s | 闭眼静息标记 |
| Resting_EO | 114 (r) | 120 (x) | duration=180s | 睁眼静息标记 |
| MainStim | 114 (r) | 120 (x) | duration=1.0s | 刺激呈现标记 (3→4) |
| MainResp | 114 (r) | 120 (x) | condition (key_resp_10.keys) | 反应窗口标记 (5→6) |
| Ending | 114 (r) | 120 (x) | duration=5s | 结束标记 |
